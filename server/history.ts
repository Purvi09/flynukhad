// Finding what happened in a city, from Wikipedia's geolocated articles.
//
// The hard part is not fetching them, it is throwing most of them away. A
// district's coordinate is the centroid of hundreds of square kilometres, so
// "fly to it" has no answer; a metro station is findable but dull. What is left
// is places you can stand in front of where something actually happened.

import { toMetres, distance, type LatLon } from "../shared/geo";
import type { Site } from "../shared/history";
import { askGeminiJson } from "./gemini";

const UA = "nukkad/0.2 (educational project)";
/** Stops two rounds sharing a street corner. */
const MIN_SEPARATION_M = 220;
...
/** Something has to have *happened* for a clue to be worth flying to. */
...
/** A region is not a place you can stand on. */
...
/** Things worth flying to: you can stand in front of them. */
...
const writeClues = async (sites: Site[]) => {
  ...
  const parsed = await askGeminiJson<{ clues?: Array<{ id?: string; clue?: string; era?: string }> }>({
    tier: "best",
    prompt:
`You write clues for a game where a player must fly to a real location in a city from memory.
...
- Write as though the player is looking down at one specific spot — a building, a monument, a corner — never a whole district or region.
...
export type HistoryResult =
  | { ok: true; sites: Site[]; source: string }
  | { ok: false; status: number; error: string };

/** The pool of cases for a city, cached for an hour. */
export const gatherHistory = async (centre: LatLon, radius: number, rounds: number): Promise<HistoryResult> => {
  const key = `${centre.lat.toFixed(3)},${centre.lon.toFixed(3)},${rounds}`;
  const hit = cache.get(key);
  if (hit && Date.now() - hit.at < CACHE_MS) return { ok: true, sites: hit.sites, source: "cache" };
  ...
  const candidates = pages
    ...
    // inside the playable area only — you cannot fly to what is not rendered
  ...
  /**
   * Only places you can physically stand on. A district has no answer, so
   * serving one as a case is worse than admitting the city is thinly recorded.
   */
  ...
  for (const separation of spacing) {
    ...
    for (const site of pool) {
      ...
      if (!chosen.some((c) => distance(c.x, c.y, site.x, site.y) < separation)) chosen.push(site);
    }
    if (chosen.length >= rounds) break;
  }
  ...
  if (chosen.length === 0) {
    return {
      ...
      error: candidates.length > 0
        ? "Only districts and townships are recorded here — nothing you could fly to. Try a larger or older city."
  ...
  return { ok: true, sites: chosen, source: written ? "gemini" : "redacted" };
};
